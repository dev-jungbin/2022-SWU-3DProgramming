...
public class PMoveY : MonoBehaviour
{
    ...
    {
        float ver = Input.GetAxis("Vertical"); // 수직 방향키 입력 (-1.0 ~ 1.0)
        float impulse = ver * power; // 힘 계산
        rb.AddRelativeForce(0, impulse, 0, ForceMode.Impulse); // X축으로 imp의 힘을 가함
    }
}
